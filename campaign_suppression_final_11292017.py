...
#changing contents in var_source column to lower case
dfMetaCampaignData_VarDef_backup = dfMetaCampaignData_VarDef_backup.withColumn("var_source", lower(col("var_source")))
#dfMetaCampaignData_VarDef_backup.select("var_source").distinct().show(10, False)

#converting string enclosed within %% to lower case
def varstruct_lower(varstruct):
    new_varstruct=''
    pct_cnt=0
    for i in varstruct:
        if i == '%':
            pct_cnt += 1
        if (pct_cnt/2)%2 == 1:
            new_varstruct+=i.lower()
        else:
            new_varstruct+=i
    return new_varstruct.strip()
varstruct_lower_udf = udf(varstruct_lower,StringType())

dfMetaCampaignData_VarDef_backup_v2 = dfMetaCampaignData_VarDef_backup.withColumn("varstructure",varstruct_lower_udf("var_structure"))
#dfMetaCampaignData_VarDef_backup_v2.select("varstructure").distinct().show(10, False)

dfMetaCampaignData_VarDef_backup = (dfMetaCampaignData_VarDef_backup_v2.drop("var_structure")
                                                                    .withColumnRenamed("varstructure","var_structure")
                                                                    )
...
dfTraveler_before_suppression = sqlContext.read.parquet(File_path)
dfTraveler_before_suppression2 = (dfTraveler_before_suppression.toDF(*[c.lower() for c in dfTraveler_before_suppression.columns]).select(tp_cols).filter("mer_status = 1"))
dfTraveler = suppressCustomersBasedOnBusinessRules(dfTraveler_before_suppression2).cache()
...
def readAllMisFiles(file_names):
	for file_name in file_names:
			 ...
			 file = file.filter(pos_filter_cond).toDF(*[c.lower() for c in file.columns])
			 file.cache()
...
try:
	dfMetaCampaignData_VarDef_backup.write.mode("overwrite").parquet(metadata_path)
except:
